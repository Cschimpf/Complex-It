...
#    }
  ...
  cluster_text <- gsub("\n", "<br>", cluster_text) # Convert newline characters to HTML line breaks
  # cluster_text <- gsub("1) Hypothesize Your Cluster Solution:", "<u><b>1) Hypothesize Your Cluster Solution:</b></u>", cluster_text) # Bold the "Purpose of Map:" heading
  # cluster_text <- gsub("2) Run the K-Means Analysis:", "<u><b>2) Run the K-Means Analysis</b></u>", cluster_text) # Bold the "Using the Map:" heading
  # # cluster_text <- gsub("Do You Have a Good Statistical Fit?", "<u><b>Do You Have a Good Statistical Fit?</b></u>", cluster_text) # Bold the "Using the Map:" heading
  # # cluster_text <- gsub("Need Help?", "<u><b>Need Help?</b></u>", cluster_text) # Bold the "Using the Map:" heading
  
  observe({
    # Check if the tab "Using Sytems Mapping To Explore Cluster Variables" is selected
    if (!is.null(input$tabs) && input$tabs == "cluster_cases" && !pop_up_clusters ) {
      shinyalert(
      ...
      pop_up_clusters <<- TRUE
    }
  })
  
  infoButton_kmean <- reactive({  input$infoButton_kmean  })
  
  observeEvent(infoButton_kmean(), {
    shinyalert(
      title = "<u><b>Using the K-Means Clusters</b></u>",
      text = cluster_text,
      size = "l",
  ...
  observe({
    # Check if the tab "Using Sytems Mapping To Explore Cluster Variables" is selected
    if (!is.null(input$tabs) && input$tabs == "AI_clusters" && !pop_up_SOM ) {
      shinyalert(
  ...
  infoButton_som <- reactive({  input$infoButton_som  })
  
  observeEvent(infoButton_som(), {
    shinyalert(
      title = "<u><b>Using the SOM AI</b></u>",
      text = train_SOM_text,
      size = "m",
...
# </div>'
  ...
  observe({
    # Check if the tab "Using Sytems Mapping To Explore Cluster Variables" is selected
    if (!is.null(input$tabs) && input$tabs == "compare_and_visualise" && !pop_up_plot_map ) {
      shinyalert(
        title = "<u><b>Using the SOM AI</b></u>",
        text = plot_map_text,
        size = "l",
  ...
  infoButton_plot_map <- reactive({  input$infoButton_plot_map  })
  
  observeEvent(infoButton_plot_map(), {
    shinyalert(
      title = "<u><b>Using the SOM AI</b></u>",
      text = plot_map_text,
      size = "l",
  ...
  observe({
    # Check if the tab "Using Sytems Mapping To Explore Cluster Variables" is selected
    if (!is.null(input$tabs) && input$tabs == "forecasting" && !pop_up_new_prediction ) {
      shinyalert(
      ...
      pop_up_new_prediction <<- TRUE
    }
  })

  infoButton_new_prediction <- reactive({  input$infoButton_new_prediction  })

  observeEvent(infoButton_new_prediction(), {
    shinyalert(
      title = "<u><b>Using the Case Prediction Tab</b></u>",
      text = new_prediction_text,
      size = "l",
  ...
  observe({
    # Check if the tab "Using Sytems Mapping To Explore Cluster Variables" is selected
    if (!is.null(input$tabs) && input$tabs == "scenarios" && !pop_up_scenarios ) {
      shinyalert(
      ...
      pop_up_scenarios <<- TRUE
    }
  })
  
  
  
  infoButton_scenarios <- reactive({  input$infoButton_scenarios  })
  
  observeEvent(infoButton_scenarios(), {
    shinyalert(
      title = "<u><b>Using the Scenarios Tab</b></u>",
      text = scenarios_text,
      size = "l",
...
#  button_pressed <- reactiveVal(FALSE)
  ...
  observe({
    # Check if the tab "Using Sytems Mapping To Explore Cluster Variables" is selected
    if (!is.null(input$tabs) && input$tabs == "systems_mapping" && !pop_up_systems ) {
      shinyalert(
